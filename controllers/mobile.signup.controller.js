...
async function signup(req, res) {
  ...
  const membershipNo = typeof req.body?.membership_no === "string" ? req.body.membership_no.trim() : "";
  const address = typeof req.body?.address === "string" ? req.body.address.trim() : "";
  const contactNumber = typeof req.body?.contact_number === "string" ? req.body.contact_number.trim() : "";
  ...
  try {
    ...
    const [[userDup]] = await conn.query("SELECT id FROM users WHERE email = ? LIMIT 1", [accountNumber]);
    if (userDup) {
      await conn.rollback();
      return res.json({ status: "fail", message: "Account number already registered", request_id: requestId, db: fp });
    }

    const [[memberDup]] = await conn.query("SELECT user_id FROM members WHERE member_code = ? LIMIT 1", [membershipNo]);
    if (memberDup) {
      await conn.rollback();
      return res.json({ status: "fail", message: "Membership number already registered", request_id: requestId, db: fp });
    }

    const [[acDup]] = await conn.query(
      "SELECT id FROM account_creation WHERE account_number = ? OR membership_no = ? LIMIT 1",
      [accountNumber, membershipNo]
    );
    if (acDup) {
      await conn.rollback();
    ...
    const [userInsert] = await conn.execute(
      `INSERT INTO users (email, password_hash, full_name, phone, is_active)
       VALUES (?, SHA2(?, 256), ?, ?, 0)`,
      [accountNumber, password, name, contactNumber]
    );
    ...
    await conn.execute(
      `INSERT INTO members (user_id, member_code, name, address, contact_number)
       VALUES (?, ?, ?, ?, ?)`,
      [userId, membershipNo, name, address, contactNumber]
    );

    const [acInsert] = await conn.execute(
      `INSERT INTO account_creation (account_number, name, membership_no, status)
       VALUES (?, ?, ?, 'pending')`,
      [accountNumber, name, membershipNo]
    );

    const [[roleRow]] = await conn.query("SELECT id FROM roles WHERE name = 'member' LIMIT 1");
    if (!roleRow) {
      await conn.rollback();
    ...
    const [[verifyUser]] = await conn.query("SELECT id, email FROM users WHERE id = ? LIMIT 1", [userId]);
...
async function health(req, res) {
  const conn = await pool.getConnection();
  try {
    const [[fp]] = await conn.query(`SELECT DATABASE() AS db_name, @@hostname AS mysql_host, @@port AS mysql_port`);
